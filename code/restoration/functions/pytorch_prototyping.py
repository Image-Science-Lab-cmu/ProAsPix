...
Original code by Vincent Sitzmann, https://github.com/vsitzmann/pytorch_prototyping

With my modifications - Vijay Rengarajan
'''
import torch
...
class ResConv2dSame(torch.nn.Module):
    '''2D convolution that pads to keep spatial dimensions equal.
    Cannot deal with stride. Only quadratic kernels (=scalar kernel_size).
    '''
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.bn3 = torch.nn.BatchNorm2d(inter_channels)
    ...
    def forward(self, x):
        x = self.relu1(self.bn3(self.conv1(x)))
        x = self.conv3(x)
        return self.relu2(self.bn1(x + self.conv1a(x)))
...
class ResUpBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if upsampling_mode == 'transpose':
            ...
            net += [nn.UpsamplingBilinear2d(scale_factor=2)]
            net += [
                ResConv2dSame(in_channels, out_channels)]
        elif upsampling_mode == 'nearest':
            net += [nn.UpsamplingNearest2d(scale_factor=2)]
            net += [
                ResConv2dSame(in_channels, out_channels)]
            ...
            net += [nn.PixelShuffle(upscale_factor=2)]
            net += [
                ResConv2dSame(in_channels // 4, out_channels)]
        else:
            raise ValueError("Unknown upsampling mode!")
        ...
        if post_conv:
            net += [ResConv2dSame(out_channels,
                               out_channels)]
...
class ResDownBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if prep_conv:
            net += [ResConv2dSame(in_channels, in_channels),
...
class ResUnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if submodule is None:
            model = [ResDownBlock(outer_nc, inner_nc, use_dropout=use_dropout, dropout_prob=dropout_prob, norm=norm),
                     ResUpBlock(inner_nc, outer_nc, use_dropout=use_dropout, dropout_prob=dropout_prob, norm=norm,
                             upsampling_mode=upsampling_mode)]
        else:
            model = [ResDownBlock(outer_nc, inner_nc, use_dropout=use_dropout, dropout_prob=dropout_prob, norm=norm),
                     submodule,
                     ResUpBlock(2 * inner_nc, outer_nc, use_dropout=use_dropout, dropout_prob=dropout_prob, norm=norm,
                             upsampling_mode=upsampling_mode)]
...
class UnetSiameseBlock(nn.Module):
    '''Helper class for building a 2D unet.
    '''
    ...
    def __init__(self,
        ...
        if submodule is None:
            self.model_down = DownBlock(outer_nc, inner_nc, use_dropout=use_dropout, dropout_prob=dropout_prob, norm=norm)
            self.model_mid = None
            self.model_up = UpBlock(inner_nc, outer_nc, use_dropout=use_dropout, dropout_prob=dropout_prob, norm=norm,
                             upsampling_mode=upsampling_mode)
        else:
            self.model_down = DownBlock(outer_nc, inner_nc, use_dropout=use_dropout, dropout_prob=dropout_prob, norm=norm)
            self.model_mid = submodule
            if self.use_skip:
                xx = 2 * inner_nc
            else:
                xx = inner_nc
            self.model_up = UpBlock(xx, outer_nc, use_dropout=use_dropout, dropout_prob=dropout_prob, norm=norm,
                             upsampling_mode=upsampling_mode)

    def forward(self, x):
        # x (B, C, P, H, W)
        for i in range(x.shape[2]):
            y = self.model_down(x[:, :, i])  # (B, C2, H2, W2)
            if i == 0:
                down = y.unsqueeze(2)  # (B, C2, 1, H2, W2)
            else:
                down = torch.cat((down, y.unsqueeze(2)), dim=2)
        # down (B, C2, P, H2, W2)

        if self.model_mid:
            y = self.model_mid(down)  # (B, C3, H3, W3)
        else:
            if self.combine_type.lower() == 'max':
                y = torch.max(down, dim=2).values # (B, C2, H2, W2)
            else:
                y = torch.mean(down, dim=2)

        y = self.model_up(y)  # (B, C, H, W)

        if self.combine_type.lower() == 'max':
            xx = torch.max(x, dim=2).values # (B, C, H, W)
        else:
            xx = torch.mean(x, dim=2)

        if self.use_skip:
            return torch.cat((xx, y), dim=1) 
...
class ResUnet(nn.Module):
    ...
    def __init__(self,
        ...
        # Define the center UNet block
        self.unet_block = ResUnetSkipConnectionBlock(min(2 ** (num_down-1) * nf0, max_channels),
                                                  min(2 ** (num_down-1) * nf0, max_channels),
                                                  use_dropout=use_dropout,
                                                  dropout_prob=dropout_prob,
                                                  norm=None, # Innermost has no norm (spatial dimension 1)
        ...
        for i in list(range(0, num_down - 1))[::-1]:
            self.unet_block = ResUnetSkipConnectionBlock(min(2 ** i * nf0, max_channels),
                                                      min(2 ** (i + 1) * nf0, max_channels),
                                                      use_dropout=use_dropout,
                                                      dropout_prob=dropout_prob,
                                                      submodule=self.unet_block,
...
class UnetCustomChan(nn.Module):
    '''A 2d-Unet implementation with sane defaults but custom channels.
    '''
    ...
    def __init__(self,
        ...
        :param in_channels: Number of input channels
        :param out_channels: Number of output channels
        :param nf: Number of features at each level of U-Net
        :param num_down: Number of downsampling stages.
        :param max_channels: Maximum number of channels (channels multiply by 2 with every downsampling stage)
        :param use_dropout: Whether to use dropout or no.
        :param dropout_prob: Dropout probability if use_dropout=True.
        ...
        # Define the in block
        self.in_layer = [Conv2dSame(in_channels, nf[0], kernel_size=3, bias=True if norm is None else False)]
        if norm is not None:
            self.in_layer += [norm(nf[0], affine=True)]
        self.in_layer += [nn.LeakyReLU(0.2, True)]
        ...
        # Define the center UNet block
        self.unet_block = UnetSkipConnectionBlock(min(nf[num_down-1], max_channels),
                                                  min(nf[num_down-1], max_channels),
                                                  use_dropout=use_dropout,
                                                  dropout_prob=dropout_prob,
                                                  norm=None, # Innermost has no norm (spatial dimension 1)
                                                  upsampling_mode=upsampling_mode)
        ...
        for i in list(range(0, num_down - 1))[::-1]:
            self.unet_block = UnetSkipConnectionBlock(min(nf[i], max_channels),
                                                      min(nf[i+1], max_channels),
                                                      use_dropout=use_dropout,
                                                      dropout_prob=dropout_prob,
                                                      submodule=self.unet_block,
                                                      norm=norm,
        ...
        # Define the out layer. Each unet block concatenates its inputs with its outputs - so the output layer
        # automatically receives the output of the in_layer and the output of the last unet layer.
        self.out_layer = [Conv2dSame(2 * nf[0],
                                     out_channels,
                                     kernel_size=3,
                                     bias=outermost_linear or (norm is None))]
...
class UnetSiamese(nn.Module):
    '''A 2d-Unet Siamese implementation with sane defaults.
    '''
    ...
    def __init__(self,
        ...
        # Define the center UNet block
        self.unet_block = UnetSiameseBlock(min(2 ** (num_down-1) * nf0, max_channels),
                                                  min(2 ** (num_down-1) * nf0, max_channels),
        ...
        for i in list(range(0, num_down - 1))[::-1]:
            self.unet_block = UnetSiameseBlock(min(2 ** i * nf0, max_channels),
                                                      min(2 ** (i + 1) * nf0, max_channels),
                                               combine_type=combine_type,
                                               use_skip=use_skip,
                                                      use_dropout=use_dropout,
        ...
        # Define the out layer. Each unet block concatenates its inputs with its outputs - so the output layer
        # automatically receives the output of the in_layer and the output of the last unet layer.
        xx = 2 * nf0 if use_skip else nf0
        self.out_layer = [Conv2dSame(xx,
                                     out_channels,
                                     kernel_size=3,
                                     bias=outermost_linear or (norm is None))]
    ...
    def forward(self, x):
        for i in range(x.shape[2]):
            in_layer_out = self.in_layer(x[:, :, i])
            if i == 0:
                in_layer = in_layer_out.unsqueeze(2)
            else:
                in_layer = torch.cat((in_layer, in_layer_out.unsqueeze(2)), dim=2)
        unet = self.unet_block(in_layer)
        out_layer = self.out_layer(unet)
        return out_layer
